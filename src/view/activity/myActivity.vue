        ...
        <div class="table-tools-column">
          <div style="float: left;">
            <el-radio-group v-model="status">
              <el-radio-button v-for="(item,index) in statusOption" :key="index" :label="item"></el-radio-button>
            </el-radio-group>
          </div>
        ...
        <el-row>
          <template v-if="tableData.length !== 0">
            ...
            <template v-for="(item,index) in tableData">
              <!-- @为v-on的缩写，:为v-bind的缩写 -->
              <el-card :key="index" style="margin: 5px 0; padding-bottom: 20px; cursor: pointer; border-radius: 10px;">
                <div style="width: 260px; height: 160px; float: left;">
                  <img style="width:100%; height:100%; border-radius: 5px;" v-lazy="formatPath(item.imagePath)" alt="加载超时" />
                </div>
                <div style="float: left; padding-left: 30px; width: 800px;">
                  <div style="font-size: 24px; font-style: italic;margin-bottom: 10px;">{{ item.activityName }}</div>
                  <div class="activity-detail-row"><i class="el-icon-user"></i> 已报名：{{item.signNum}}/{{item.peopleNum}}人</div>
                  <div class="activity-detail-row">
                    <el-button style="float: right;" type="primary">查看报名名单</el-button>
                    <el-button style="float: right; margin-right: 15px;" @click.native="rowClick(item)">修改活动信息</el-button>
                  </div>
                </div>
              </el-card>
            </template>
...
export default {
  ...
  data () {
    return {
      ...
      status: '全部',
      statusOption: ['全部', '待审核', '已审核', '报名阶段', '待完结', '已完结']
    }
  },
  methods: {
    // 返回
